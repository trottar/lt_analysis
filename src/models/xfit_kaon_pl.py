...
# Description:
# ================================================================
# Time-stamp: "2024-10-07 09:37:28 trottar"
# ================================================================
#
...
def prepare_equations(equations, sig_type):
    if sig_type == "sig_L":
        ...
        func_str = f"def {sig_type}_optimized(q2_set, w_set, tt, p1, p2, p3, p4):\n"
    if sig_type == "sig_T":
        eq_list = [f"{k} = {v}" for k, v in equations.items() if k not in ('sig_L', 'sig_LT', 'sig_TT')]
        func_str = f"def {sig_type}_optimized(q2_set, w_set, tt, p5, p6, p7, p8):\n"
    if sig_type == "sig_LT":
        eq_list = [f"{k} = {v}" for k, v in equations.items() if k not in ('sig_L', 'sig_T', 'sig_TT')]
        func_str = f"def {sig_type}_optimized(q2_set, w_set, tt, p9, p10, p11, p12):\n"
    if sig_type == "sig_TT":
        eq_list = [f"{k} = {v}" for k, v in equations.items() if k not in ('sig_L', 'sig_T', 'sig_LT')]
        func_str = f"def {sig_type}_optimized(q2_set, w_set, tt, p13, p14, p15, p16):\n"
...
def fun_Sig_L(x, par):
    ...
    return fun_Sig_L_optimized(q2_set, w_set, tt, p1, p2, p3, p4)
...
def fun_Sig_T(x, par):
    ...
    return fun_Sig_T_optimized(q2_set, w_set, tt, p5, p6, p7, p8)
...
def fun_Sig_LT(x, par):
    ...
    return fun_Sig_LT_optimized(q2_set, w_set, tt, p9, p10, p11, p12)
...
def fun_Sig_TT(x, par):
    ...
    return fun_Sig_TT_optimized(q2_set, w_set, tt, p13, p14, p15, p16)
