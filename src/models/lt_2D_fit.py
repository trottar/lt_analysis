...
def single_setting(q2_set, w_set, fn_lo, fn_hi):
    ...
    for i in range(0, t_bin_num):    
        ...
        fff2.SetParError(2, 0.5)                        # ρ_LT step
        fff2.SetParError(3, 0.5)                        # ρ_TT step  
        g_plot_err.Fit(fff2, "MWLSV")       # quiet, no redraw
...
g_sig_l_total.Fit(f_exp_l, "MWLSV")
g_sig_t_total.Fit(f_exp_t, "MWLSV")
...
g_sig_t_total.Fit(f_exp, "MWLSV")
c_total.Print(outputpdf)
c_total.Clear()

g_sig_lt_total.Draw("A*")
g_sig_lt_total.Fit(f_exp, "MWLSV")
c_total.Print(outputpdf)
c_total.Clear()

g_sig_tt_total.Draw("A*")
g_sig_tt_total.Fit(f_exp, "MWLSV")
c_total.Print(outputpdf+')')
c_total.Clear()
