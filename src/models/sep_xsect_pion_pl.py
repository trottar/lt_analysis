...
def import_model(inp_model, arg_str):
    ...
    Q2, sin_theta_cm, tt, qq, ww, *params = args
    ...
    # Function for SigLT
    # sin_theta_cm term is defined on function calling
    def sig_LT(*par):
        if inp_model == "sigLT":
            ...
            try:
                f_tt=abs(tt)/(abs(tt)+mkpl**2)**2 # pole factor
                ##
                f = (par[0]*math.exp(par[1]*abs(tt))+par[2]/abs(tt))*sin_theta_cm
    ...
    def sig_TT(*par):
        if inp_model == "sigTT":
            ...
            try:
                ...
                f_tt=abs(tt)/(abs(tt)+mkpl**2)**2 # pole factor
                ##
                f = (par[0]*qq*math.exp(-qq))*f_tt*(sin_theta_cm**2)
                # RLT (4/23/2024): Marco's thesis functional forms
                #f = par[0] * math.exp(-par[1]*abs(tt)) * (1.0 / (1 + (qq**2)*par[2]))
                ##
                # RLT (7/11/2024): Redefined functional forms of L, T, LT, TT
