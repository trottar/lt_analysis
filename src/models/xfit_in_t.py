...
# Description:
# ================================================================
# Time-stamp: "2024-01-28 04:44:57 trottar"
# ================================================================
#
...
def single_setting(ParticleType, pol_str, dir_iter, q2_set, w_set, tmin_range, tmax_range):
    ...
    for i in range(len(w_vec)):
        ...
        g_q2_sigl_fit.SetPointError(g_q2_sigl_fit.GetN()-1, g_sigl_fit.GetEX()[i], 0.0, g_sigl_fit.GetEY()[i] / g_vec[i])
        print("&&&&",g_q2_sigl_fit.GetN(), q2_vec[i], g_sigl_fit.GetX()[i], g_sigl_fit.GetY()[i])
        print((l0+l1*math.log(q2_vec[i])) * math.exp((l2+l3*math.log(q2_vec[i])) * (abs(g_sigl_fit.GetX()[i]))) / g_vec[i], " =!= ", g_sigl_fit.GetY()[i])
