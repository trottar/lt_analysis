...
# Description:
# ================================================================
# Time-stamp: "2024-02-06 01:50:49 trottar"
# ================================================================
#
...
def single_setting(ParticleType, pol_str, dir_iter, q2_set, w_set, tmin_range, tmax_range, Q2min_range, Q2max_range):
    ...
    for i in range(len(w_vec)):
        ...
        sigt_X_fit = (g_sigt.GetY()[i]) / (g_vec[i]/2/PI/1e6)
        sigt_X_fit_err = g_sigt.GetEY()[i] / (g_vec[i]/2/PI/1e6)
    ...
    for i in range(len(w_vec)):
        ...
        if th_vec[i] == 180:
            ...
            siglt_X_fit = g_siglt.GetY()[i] / ((g_vec[i]/2/PI/1e6) * math.sin(th_vec[i] * PI / 180))
            siglt_X_fit_err = g_siglt.GetEY()[i] / ((g_vec[i]/2/PI/1e6) * math.sin(th_vec[i] * PI / 180))
    ...
    for i in range(len(w_vec)):
        ...
        if th_vec[i] == 180:
            ...
            sigtt_X_fit = g_sigtt.GetY()[i] / ((g_vec[i]/2/PI/1e6) * math.sin(th_vec[i] * PI / 180)**2)
            sigtt_X_fit_err = g_sigtt.GetEY()[i] / ((g_vec[i]/2/PI/1e6) * math.sin(th_vec[i] * PI / 180)**2)
    ...
    with open(para_file_out, 'w') as f:
        ...
        for i in range(len(par_vec)):
            ...
            #f.write("{:13.5e} {:13.5e} {:3d} {:12.1f}\n".format(prv_par_vec[i], prv_par_err_vec[i], i, prv_par_chi2_vec[i]))
            print("  {:.3f} {:.3f} {:.1f} {:.1f}".format(prv_par_vec[i], prv_par_err_vec[i], i, prv_par_chi2_vec[i]))
