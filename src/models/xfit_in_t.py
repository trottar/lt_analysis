...
# Description:
# ================================================================
# Time-stamp: "2024-02-09 01:59:21 trottar"
# ================================================================
#
...
def single_setting(ParticleType, pol_str, dir_iter, q2_set, w_set, tmin_range, tmax_range, Q2min_range, Q2max_range):
    ...
    for i in range(len(w_vec)):
        ...
        sigl_X_pre = (f_sigL_pre.Eval(g_sigl.GetX()[i], q2_vec[i])) * (g_vec[i]/2./PI)
        g_sigl_prv.SetPoint(i, g_sigl.GetX()[i], sigl_X_pre)

        sigl_X_fit = g_sigl.GetY()[i] / (g_vec[i]/2./PI)
        sigl_X_fit_err = g_sigl.GetEY()[i] / (g_vec[i]/2./PI)
    ...
    for i in range(len(w_vec)):
        ...
        sigt_X_pre = (f_sigT_pre.Eval(g_sigt.GetX()[i], q2_vec[i])) * (g_vec[i]/2./PI)
        g_sigt_prv.SetPoint(i, g_sigt.GetX()[i], sigt_X_pre)

        sigt_X_fit = (g_sigt.GetY()[i]) / (g_vec[i]/2./PI)
        sigt_X_fit_err = g_sigt.GetEY()[i] / (g_vec[i]/2./PI)
    ...
    for i in range(len(w_vec)):
        ...
        sigt_X = (f_sigT.Eval(g_sigt.GetX()[i], q2_vec[i])) * (g_vec[i]/2./PI)
    ...
    for i in range(len(w_vec)):
        ...
        siglt_X_pre = (f_sigLT_pre.Eval(g_siglt.GetX()[i], q2_vec[i]) * math.sin(th_vec[i] * PI / 180)) * (g_vec[i]/2./PI)
        ...
        if th_vec[i] == 180:
            ...
            siglt_X_fit = g_siglt.GetY()[i] / ((g_vec[i]/2./PI) * math.sin(th_vec[i] * PI / 180))
            siglt_X_fit_err = g_siglt.GetEY()[i] / ((g_vec[i]/2./PI) * math.sin(th_vec[i] * PI / 180))
    ...
    for i in range(len(w_vec)):
        ...
        siglt_X = (f_sigLT.Eval(g_siglt.GetX()[i], q2_vec[i]) * math.sin(th_vec[i] * PI / 180)) * (g_vec[i]/2./PI)
    ...
    for i in range(len(w_vec)):
        ...
        sigtt_X_pre = (f_sigTT_pre.Eval(g_sigtt.GetX()[i], q2_vec[i]) * math.sin(th_vec[i] * PI / 180)**2) * (g_vec[i]/2./PI)
        ...
        if th_vec[i] == 180:
            ...
            sigtt_X_fit = g_sigtt.GetY()[i] / ((g_vec[i]/2./PI) * math.sin(th_vec[i] * PI / 180)**2)
            sigtt_X_fit_err = g_sigtt.GetEY()[i] / ((g_vec[i]/2./PI) * math.sin(th_vec[i] * PI / 180)**2)
    ...
    for i in range(len(w_vec)):
        ...
        sigtt_X = (f_sigTT.Eval(g_sigtt.GetX()[i], q2_vec[i]) * math.sin(th_vec[i] * PI / 180)**2) * (g_vec[i]/2./PI)
