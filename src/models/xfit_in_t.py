...
# Description:
# ================================================================
# Time-stamp: "2024-02-09 02:17:16 trottar"
# ================================================================
#
...
def single_setting(ParticleType, pol_str, dir_iter, q2_set, w_set, tmin_range, tmax_range, Q2min_range, Q2max_range):
    ...
    for i in range(len(w_vec)):
        ...
        siglt_X_pre = (f_sigLT_pre.Eval(g_siglt.GetX()[i], q2_vec[i]) * math.sin(th_vec[i] )) * (1.0)
        ...
        if th_vec[i] == 180:
            ...
            siglt_X_fit = g_siglt.GetY()[i] / ((1.0) * math.sin(th_vec[i] ))
            siglt_X_fit_err = g_siglt.GetEY()[i] / ((1.0) * math.sin(th_vec[i] ))
    ...
    for i in range(len(w_vec)):
        ...
        siglt_X = (f_sigLT.Eval(g_siglt.GetX()[i], q2_vec[i]) * math.sin(th_vec[i] )) * (1.0)
    ...
    for i in range(len(w_vec)):
        ...
        sigtt_X_pre = (f_sigTT_pre.Eval(g_sigtt.GetX()[i], q2_vec[i]) * math.sin(th_vec[i] )**2) * (1.0)
        ...
        if th_vec[i] == 180:
            ...
            sigtt_X_fit = g_sigtt.GetY()[i] / ((1.0) * math.sin(th_vec[i] )**2)
            sigtt_X_fit_err = g_sigtt.GetEY()[i] / ((1.0) * math.sin(th_vec[i] )**2)
    ...
    for i in range(len(w_vec)):
        ...
        sigtt_X = (f_sigTT.Eval(g_sigtt.GetX()[i], q2_vec[i]) * math.sin(th_vec[i] )**2) * (1.0)
