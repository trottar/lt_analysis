...
# Description:
# ================================================================
# Time-stamp: "2024-02-05 17:54:22 trottar"
# ================================================================
#
...
def single_setting(ParticleType, pol_str, dir_iter, q2_set, w_set, tmin_range, tmax_range):
    ...
    for i in range(len(w_vec)):
        ...
        if th_vec[i] != 180:
            siglt_X_fit = g_siglt.GetY()[i] / g_vec[i] / math.sin(th_vec[i] * PI / 180) * (2/PI/1e6)
            siglt_X_fit_err = g_siglt.GetEY()[i] / g_vec[i] / math.sin(th_vec[i] * PI / 180) * (2/PI/1e6)
        else:
            siglt_X_fit = 0.0
    ...
    for i in range(len(w_vec)):
        ...
        if th_vec[i] != 180:
            sigtt_X_fit = g_sigtt.GetY()[i] / g_vec[i] / math.sin(th_vec[i] * PI / 180) / math.sin(th_vec[i] * PI / 180) * (2/PI/1e6)
            sigtt_X_fit_err = g_sigtt.GetEY()[i] / g_vec[i] / math.sin(th_vec[i] * PI / 180) / math.sin(th_vec[i] * PI / 180) * (2/PI/1e6)
        else:
            sigtt_X_fit = 0.0
