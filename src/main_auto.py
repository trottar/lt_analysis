...
# Description:
# ================================================================
# Time-stamp: "2025-04-17 16:17:06 trottar"
# ================================================================
#
...
if len(sys.argv)-1!=14:
    print("!!!!! ERROR !!!!!\n Expected 14 arguments\n Usage is with - KIN W Q2 LOEPS HIEPS ParticleType EPSSET POL OutFilename formatted_date NumtBins NumPhiBins inp_debug RATIO_THRESHOLD_SPREAD\n!!!!! ERROR !!!!!")
    sys.exit(1)
...
if inpDict.get("particle_subtraction_mode") == "simc_shape_components":
    for hist in histlist:
        component_payload = load_setting_pion_component_shapes(
            ...
            context="main_auto_step5_pre_compare",
        )
        ...
        if ParticleType == "kaon":
            ...
            kaon_sigma0_payload = load_kaon_simc_sigma0_shape(
                ...
                t_bins=hist.get("t_bins"),
                phi_bins=hist.get("phi_bins"),
                context="main_auto_step5_pre_compare_sigma0",
            )
        ...
        created_plot = plot_pion_component_background_payload(
            ...
            inpDict,
            component_plot_path,
            kaon_sigma0_payload=kaon_sigma0_payload,
        )
        if created_plot:
...
if EPSSET == "high":
    ...
    try:
        ...
        run_bash_script('{}/run_xsect.sh'.format(LTANAPATH), Q2, W, ParticleType, POL, str(inpDict["NumtBins"]), str(inpDict["NumPhiBins"]), str(iter_num))        
...
if EPSSET == "high":
    ...
    if request_yn_response(string=f"Updating {new_dir_cache} with {new_dir}..."):
        ...
        subprocess.call(f"jput {f_str} {new_dir_cache}", shell=True) # HERE!!!!!!
    else:
...
if EPSSET == "high":
    ...
    if iter_num > 1:        
        ...
        print("-"*50)
        print(f"\nStep 1: Checking ratio for consistency with unity and threshold spread of {RATIO_THRESHOLD_SPREAD*100:.1f}\n")
        CONTINUE = check_ratio(inpDict, new_dir, RATIO_THRESHOLD_SPREAD)
        if CONTINUE:
            c+=1
            sys.path.append("../analysis_monitor/monitor")
            from kinematics_checker import check_kinematics
