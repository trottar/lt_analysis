...
# Description:
# ================================================================
# Time-stamp: "2023-08-24 23:54:33 trottar"
# ================================================================
#
...
for phiset in phisetlist:
    for k, data_key_tuple in enumerate(averDict["binned_DATA"][phiset]['t']):
        ...
        histbinDict["H_Q2_tbin_DATA_{}_{}_{}".format(phiset, str(i+1), str(j+1))] = ROOT.TH1D("H_Q2_tbin_DATA_{}_{}_{}".format(phiset, str(i+1), str(j+1)), "{} Data Q2 (t bin {}, phi bin {})".format(phiset, str(i+1), str(j+1)), 500, 0, inpDict["Q2max"])
        histbinDict["H_W_tbin_DATA_{}_{}_{}".format(phiset, str(i+1), str(j+1))] = ROOT.TH1D("H_W_tbin_DATA_{}_{}_{}".format(phiset, str(i+1), str(j+1)), "{} Data W (t bin {}, phi bin {})".format(phiset, str(i+1), str(j+1)), 500, 0, inpDict["Wmax"])
        histbinDict["H_t_tbin_DATA_{}_{}_{}".format(phiset, str(i+1), str(j+1))] = ROOT.TH1D("H_t_tbin_DATA_{}_{}_{}".format(phiset, str(i+1), str(j+1)), "{} Data t (t bin {}, phi bin {})".format(phiset, str(i+1), str(j+1)), 500, 0, inpDict["tmax"]) 
...
for phiset in phisetlist:
    for k, simc_key_tuple in enumerate(averDict["binned_SIMC"][phiset]['t']):
        ...
        histbinDict["H_Q2_tbin_SIMC_{}_{}_{}".format(phiset, str(i+1), str(j+1))] = ROOT.TH1D("H_Q2_tbin_SIMC_{}_{}_{}".format(phiset, str(i+1), str(j+1)), "{} Simc Q2 (t bin {}, phi bin {})".format(phiset, str(i+1), str(j+1)), 500, 0, inpDict["Q2max"])
        histbinDict["H_W_tbin_SIMC_{}_{}_{}".format(phiset, str(i+1), str(j+1))] = ROOT.TH1D("H_W_tbin_SIMC_{}_{}_{}".format(phiset, str(i+1), str(j+1)), "{} Simc W (t bin {}, phi bin {})".format(phiset, str(i+1), str(j+1)), 500, 0, inpDict["Wmax"])
        histbinDict["H_t_tbin_SIMC_{}_{}_{}".format(phiset, str(i+1), str(j+1))] = ROOT.TH1D("H_t_tbin_SIMC_{}_{}_{}".format(phiset, str(i+1), str(j+1)), "{} Simc t (t bin {}, phi bin {})".format(phiset, str(i+1), str(j+1)), 500, 0, inpDict["tmax"])
...
for it,phiset in enumerate(phisetlist):
    ...
    for k, simc_key_tuple in enumerate(simc_key_tuples):
        ...
        for (itt,jtt), val in np.ndenumerate(simc_nested_dict["Q2"][(i,j)]["Q2_arr"]):
            histbinDict["H_Q2_tbin_SIMC_{}_{}_{}".format(phiset,str(i+1),str(j+1))].Fill((itt+jtt)/2, val)
...
for it,phiset in enumerate(phisetlist):
    ...
    for k, data_key_tuple in enumerate(data_key_tuples):
        ...
        for (itt,jtt), val in np.ndenumerate(data_nested_dict["W"][(i,j)]["W_arr"]):
            histbinDict["H_W_tbin_DATA_{}_{}_{}".format(phiset,str(i+1),str(j+1))].Fill((itt+jtt)/2, val)
...
for it,phiset in enumerate(phisetlist):
    ...
    for k, simc_key_tuple in enumerate(simc_key_tuples):
        ...
        for (itt,jtt), val in np.ndenumerate(simc_nested_dict["W"][(i,j)]["W_arr"]):
            histbinDict["H_W_tbin_SIMC_{}_{}_{}".format(phiset,str(i+1),str(j+1))].Fill((itt+jtt)/2, val)
...
for it,phiset in enumerate(phisetlist):
    ...
    for k, data_key_tuple in enumerate(data_key_tuples):
        ...
        for (itt,jtt), val in np.ndenumerate(data_nested_dict["t"][(i,j)]["t_arr"]):
            histbinDict["H_t_tbin_DATA_{}_{}_{}".format(phiset,str(i+1),str(j+1))].Fill((itt+jtt)/2, val)
...
for it,phiset in enumerate(phisetlist):
    ...
    for k, simc_key_tuple in enumerate(simc_key_tuples):
        ...
        for (itt,jtt), val in np.ndenumerate(simc_nested_dict["t"][(i,j)]["t_arr"]):
            histbinDict["H_t_tbin_SIMC_{}_{}_{}".format(phiset,str(i+1),str(j+1))].Fill((itt+jtt)/2, val)
...
for simc_key_tuple in averDict["binned_SIMC"]:
    i = simc_key_tuple[0] # t bin
    j = simc_key_tuple[1] # phi bin
    histbinDict["H_Q2_tbin_SIMC_{}_{}".format(str(i+1),str(j+1))] = ROOT.TH1D("H_Q2_tbin_SIMC_{}_{}".format(str(i+1),str(j+1)), "Simc Q2 (t bin {}, phi bin {})".format(str(i+1),str(j+1)), 500, inpDict["Q2min"], inpDict["Q2max"])
    histbinDict["H_W_tbin_SIMC_{}_{}".format(str(i+1),str(j+1))] = ROOT.TH1D("H_W_tbin_SIMC_{}_{}".format(str(i+1),str(j+1)), "Simc W (t bin {}, phi bin {})".format(str(i+1),str(j+1)), 500, inpDict["Wmin"], inpDict["Wmax"])
    histbinDict["H_t_tbin_SIMC_{}_{}".format(str(i+1),str(j+1))] = ROOT.TH1D("H_t_tbin_SIMC_{}_{}".format(str(i+1),str(j+1)), "Simc t (t bin {}, phi bin {})".format(str(i+1),str(j+1)), 500, inpDict["tmin"], inpDict["tmax"])
...
for k, data_key_tuple in enumerate(averDict["binned_DATA"]):
    ...
    for val in data_nested_dict["Q2_arr"]:
        histbinDict["H_Q2_tbin_DATA_{}_{}".format(str(i+1),str(j+1))].Fill(val)
    C_Q2_tbin_DATA.cd(k+1)
    histbinDict["H_Q2_tbin_DATA_{}_{}".format(str(i+1),str(j+1))].Draw("same")
...
for k, simc_key_tuple in enumerate(averDict["binned_SIMC"]):
    ...
    for val in simc_nested_dict["Q2_arr"]:
        histbinDict["H_Q2_tbin_SIMC_{}_{}".format(str(i+1),str(j+1))].Fill(val)
    C_Q2_tbin_SIMC.cd(k+1)
    histbinDict["H_Q2_tbin_SIMC_{}_{}".format(str(i+1),str(j+1))].Draw("same")
...
for k, data_key_tuple in enumerate(averDict["binned_DATA"]):
    ...
    for val in data_nested_dict["W_arr"]:
        histbinDict["H_W_tbin_DATA_{}_{}".format(str(i+1),str(j+1))].Fill(val)
    C_W_tbin_DATA.cd(k+1)
    histbinDict["H_W_tbin_DATA_{}_{}".format(str(i+1),str(j+1))].Draw("same")
...
for k, simc_key_tuple in enumerate(averDict["binned_SIMC"]):
    ...
    for val in simc_nested_dict["W_arr"]:
        histbinDict["H_W_tbin_SIMC_{}_{}".format(str(i+1),str(j+1))].Fill(val)
    C_W_tbin_SIMC.cd(k+1)
    histbinDict["H_W_tbin_SIMC_{}_{}".format(str(i+1),str(j+1))].Draw("same")
...
for k, data_key_tuple in enumerate(averDict["binned_DATA"]):
    ...
    for val in data_nested_dict["t_arr"]:
        histbinDict["H_t_tbin_DATA_{}_{}".format(str(i+1),str(j+1))].Fill(val)
    C_t_tbin_DATA.cd(k+1)
    histbinDict["H_t_tbin_DATA_{}_{}".format(str(i+1),str(j+1))].Draw("same")
...
for k, simc_key_tuple in enumerate(averDict["binned_SIMC"]):
    ...
    for val in simc_nested_dict["t_arr"]:
        histbinDict["H_t_tbin_SIMC_{}_{}".format(str(i+1),str(j+1))].Fill(val)
    C_t_tbin_SIMC.cd(k+1)
    histbinDict["H_t_tbin_SIMC_{}_{}".format(str(i+1),str(j+1))].Draw("same")
