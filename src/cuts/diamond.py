...
def diamond_fit(Q2vsW_hist, Q2Val, fitrange=10):
    ...
    fitl = Q2vsW_hist.GetXaxis().FindBin(Q2Val) - int(fitrange*10)
...
def DiamondPlot(ParticleType, Q2Val, Q2min, Q2max, WVal, Wmin, Wmax, phi_setting, tmin, tmax, inpDict):
	...
	##############################################################################################################################################
        ...
        if (k==2): # High
            apply_bin_threshold(Q2vsW_cut, 15)
            apply_bin_threshold(Q2vsW_hi_cut, 15)
        elif (k==1): # Mid
            apply_bin_threshold(Q2vsW_mide_cut, 15)
            apply_bin_threshold(Q2vsW_mi_cut, 15)
        elif (k==0): # Low
            apply_bin_threshold(Q2vsW_lowe_cut, 15)
            apply_bin_threshold(Q2vsW_lo_cut, 15)
