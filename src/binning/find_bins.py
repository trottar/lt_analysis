...
# Description:
# ================================================================
# Time-stamp: "2024-12-05 07:08:21 trottar"
# ================================================================
#
...
def find_bins(histlist, inpDict):
    ...
    def find_tbins(H_t_BinTest):
        ...
        def adjust_bins(x, nbin, tolerance=1e-3, max_iterations=10):
            ...
            # Helper function to calculate edge scaling based on position
            def edge_weight_factor(pos, total_bins):
                """Adjust weighting to move edges faster near dataset boundaries."""
                rel_position = pos / (total_bins - 1)  # Normalize position to range [0, 1]
                return 1.0 + 3.0 * (1 - rel_position) ** 2  # Parabolic scaling, 3x at edges
            ...
            while np.any(counts < bad_bins_threshold):
                ...
                for i in range(1, len(bin_edges) - 1):
                    if counts[i - 1] < bad_bins_threshold:
                        # Increase bin edge to meet minimum events
                        bin_edges[i] += (tolerance / 2) * edge_weight_factor(i, nbin)
                    elif counts[i] < bad_bins_threshold:
                        # Decrease bin edge to meet minimum events
                        bin_edges[i] -= (tolerance / 2) * edge_weight_factor(i, nbin)
                ...
                for it in range(max_iterations):
                    ...
                    for j, exceed in enumerate(exceed_tolerance):
                        if exceed:
                            weight_factor = edge_weight_factor(j + 1, nbin)
                            if diff_counts[j] > 0:
                                # Increase bin edge
                                bin_edges[j + 1] += (tolerance / 2) * weight_factor
                            else:
                                # Decrease bin edge
                                bin_edges[j + 1] -= (tolerance / 2) * weight_factor
