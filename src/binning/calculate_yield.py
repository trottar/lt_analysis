...
# Description:
# ================================================================
# Time-stamp: "2024-03-21 20:32:36 trottar"
# ================================================================
#
...
def process_hist_data(tree_data, tree_dummy, t_bins, phi_bins, nWindows, phi_setting, inpDict):
            ...
            for j in range(len(t_bins)-1):
                for k in range(len(phi_bins)-1):            
                    if t_bins[j]+0.01 < -evt.MandelT < t_bins[j+1]+0.01:
                        if phi_bins[k]+1.0 < (evt.ph_q+math.pi)*(180 / math.pi) < phi_bins[k+1]+1.0:
    ...
    for j in range(len(t_bins)-1):
        for k in range(len(phi_bins)-1):
            ...
            processed_dict["t_bin{}phi_bin{}".format(j+1,k+1)] = sorted(processed_dict["t_bin{}phi_bin{}".format(j+1,k+1)].keys())
