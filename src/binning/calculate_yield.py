...
def _assert_canonical_t_edges(inpDict, t_bins, stage, phi_bins=None):
    ...
    if not (matched and phi_matched) and bool(get_proton_contamination_cleaning_config(inp_dict=inpDict).get("strict_mode", False)):
...
def _clone_hist_for_plot(hist, scope="yield", role="display"):
...
def _apply_component_pion_subtraction_for_bin(
    ...
    print_particle_subtraction_weight_support_warning(
        weight_payload,
        context="calculate_yield component pion subtraction",
...
def process_hist_data(
    ...
    for j in range(len(t_bins)-1):
        for k in range(len(phi_bins)-1):
            ...
            dummy_norm_hist_dict["H_MM_DUMMY_NORM_{}_{}".format(j, k)] = clone_root_histogram(
                hist_bin_dict["H_MM_DUMMY_{}_{}".format(j, k)],
                scope="yield_t{}_phi{}".format(j + 1, k + 1),
                role="dummy_normalized",
                name="H_MM_DUMMY_NORM_{}_{}".format(j, k),
                sumw2=False,
            )
                    ...
                    if "MM_DATA" in key:
                        ...
                        if isinstance(component_payload, dict):
                            print_particle_subtraction_component_application_pages(
                                ...
                                title_prefix="{} t{} phi{}".format(phi_setting, j + 1, k + 1),
                                cut_window=(float(inpDict["mm_min"]), float(inpDict["mm_max"])),
                                component_fit_result=component_fit_results[j][k],
                            )
...
def bin_data(
    ...
    for j in range(len(t_bins)-1):
        for k in range(len(phi_bins)-1):
            ...
            mm_hist_data.append(_clone_hist_for_plot(H_MM_DATA, scope="yield", role="data_mm"))
            mm_hist_dummy_norm.append(_clone_hist_for_plot(H_MM_DUMMY_NORM, scope="yield", role="dummy_mm"))
            mm_hist_sub.append(_clone_hist_for_plot(H_MM_SUB_DATA, scope="yield", role="subtracted_mm"))
...
def find_yield_data(histlist, inpDict):
    ...
    _assert_canonical_t_edges(inpDict, t_bins, "calculate_yield.find_yield_data", phi_bins)
...
def bin_simc(kin_type, tree_simc, normfac_simc, t_bins, phi_bins, phi_setting, inpDict, iteration):
    ...
    for j in range(len(t_bins)-1):
        for k in range(len(phi_bins)-1):
            ...
            mm_hist_simc.append(_clone_hist_for_plot(H_MM_SIMC, scope="yield", role="simc_mm"))
...
def find_yield_simc(histlist, inpDict, iteration=False):
    ...
    for hist in histlist:
        t_bins = hist["t_bins"]
        phi_bins = hist["phi_bins"]
    _assert_canonical_t_edges(inpDict, t_bins, "calculate_yield.find_yield_simc", phi_bins)
