...
# Description:
# ================================================================
# Time-stamp: "2023-09-27 21:10:25 trottar"
# ================================================================
#
...
for k in range(NumtBins):
    ...
    for i in range(0,NumtBins*NumPhiBins-1):
        print(np.array(file_df_dict['unsep_file_loeps']['tm'].tolist())[i]," - ",t_bin_centers[k])
        if are_within_tolerance(np.array(file_df_dict['unsep_file_loeps']['tm'].tolist())[i],t_bin_centers[k],np.array(file_df_dict['unsep_file_loeps']['Q2'].tolist())[i]):
            print("^^")
            if bin_check:
                break
            else:
