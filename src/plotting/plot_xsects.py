...
# Description:
# ================================================================
# Time-stamp: "2024-03-23 10:49:10 trottar"
# ================================================================
#
...
with PdfPages(outputpdf) as pdf:
    ...
    for k in range(NumtBins):
        ...
        for i, df_key in enumerate(['aver_loeps', 'aver_hieps']):
            ...
            mask = (df['tbin'] == (k+1))
            ax.errorbar(phi_bin_centers[df['phibin'][mask]], df['ratio'][mask], yerr=df['dratio'][mask], marker=markers[i], linestyle='None', label=df_key, facecolors='none' , edgecolors=colors[i])
    ...
    for k in range(NumtBins):
        ...
        for i, df_key in enumerate(['unsep_file_loeps', 'unsep_file_hieps']):
            ...
            ax.scatter(df['phi'][mask], df['Q2'][mask], marker=markers[i], linestyle='None', label=df_key, facecolors='none' , edgecolors=colors[i])
    ...
    for k in range(NumtBins):
        ...
        for i, df_key in enumerate(['unsep_file_loeps', 'unsep_file_hieps']):
            df = file_df_dict[df_key]
            mask =  (df['t'][k*NumPhiBins+int(i/NumPhiBins)] == df['t'])
            ax.scatter(df['phi'][mask], df['W'][mask], marker=markers[i], linestyle='None', label=df_key, facecolors='none' , edgecolors=colors[i])
    ...
    for k in range(NumtBins):
        ...
        for i, df_key in enumerate(['unsep_file_loeps', 'unsep_file_hieps']):
            df = file_df_dict[df_key]
            mask =  (df['t'][k*NumPhiBins+int(i/NumPhiBins)] == df['t'])
            ax.errorbar(df['th_cm'][mask], df['x_real'][mask], yerr=df['dx_real'][mask], marker=markers[i], linestyle='None', label=df_key, facecolors='none' , edgecolors=colors[i])
    ...
    for k in range(NumtBins):
        ...
        for i, df_key in enumerate(['unsep_file_loeps', 'unsep_file_hieps']):
            df = file_df_dict[df_key]
            mask =  (df['t'][k*NumPhiBins+int(i/NumPhiBins)] == df['t'])
            ax.errorbar(df['phi'][mask], df['x_real'][mask], yerr=df['dx_real'][mask], marker=markers[i], linestyle='None', label=df_key, facecolors='none' , edgecolors=colors[i])
