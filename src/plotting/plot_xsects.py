...
# Description:
# ================================================================
# Time-stamp: "2024-03-23 11:12:41 trottar"
# ================================================================
#
...
with PdfPages(outputpdf) as pdf:
    ...
    for k in range(NumtBins):
        ...
        for i, df_key in enumerate(['aver_loeps', 'aver_hieps']):
            ...
            mask = (df['tbin'] == (k+1))
            ax.errorbar(phi_bin_centers[df['phibin'][mask]], df['ratio'][mask], yerr=df['dratio'][mask], marker=markers[i], linestyle='None', label=df_key, color=colors[i], markerfacecolor='none', capsize=2)
    ...
    for k in range(NumtBins):
        ...
        for i, df_key in enumerate(['unsep_file_loeps', 'unsep_file_hieps']):
            ...
            mask =  (df['t'][k*NumPhiBins+int(i/NumPhiBins)] == df['t'])
            ax.errorbar(df['th_cm'][mask], df['x_real'][mask], yerr=df['dx_real'][mask], marker=markers[i], linestyle='None', label=df_key, color=colors[i], markerfacecolor='none', capsize=2)
    ...
    for k in range(NumtBins):
        ...
        for i, df_key in enumerate(['unsep_file_loeps', 'unsep_file_hieps']):
            ...
            mask =  (df['t'][k*NumPhiBins+int(i/NumPhiBins)] == df['t'])
            ax.errorbar(df['phi'][mask], df['x_real'][mask], yerr=df['dx_real'][mask], marker=markers[i], linestyle='None', label=df_key, color=colors[i], markerfacecolor='none', capsize=2)
    ...
    for k, sig in enumerate(['sigL','sigT','sigLT','sigTT']):
        ...
        for i, df_key in enumerate(['sep_file']):
            ...
            ax.errorbar(df['t'], df['{}'.format(sig)], yerr=df['d{}'.format(sig)], marker=markers[i], linestyle='None', label='Data', color=colors[i], markerfacecolor='none', capsize=2)
        ax.set_xlabel('t')
