...
# Description:
# ================================================================
# Time-stamp: "2024-07-30 14:48:42 trottar"
# ================================================================
#
...
with PdfPages(outputpdf) as pdf:
    ...
    for k in range(NumtBins):
        ...
        for i, df_key in enumerate(['unsep_file_loeps', 'unsep_file_hieps']):
            ...
            if "hi" in df_key:
                ...
                b_hi_lst.append(b_fit*(df['W'][mask][non_zero_mask].to_numpy()-float(W.replace("p","."))))
                c_hi_lst.append(c_fit)
                d_hi_lst.append(d_fit)
            else:
                a_lo_lst.append(a_fit)
                b_lo_lst.append(b_fit*(df['W'][mask][non_zero_mask].to_numpy()-float(W.replace("p","."))))
    ...
    for k in range(NumPhiBins):
        ...
        for i, df_key in enumerate(['unsep_file_loeps', 'unsep_file_hieps']):
            ...
            mask =  (df['phi'][k*NumtBins+int(i/NumtBins)] == df['phi'])
    ...
    for k in range(NumPhiBins):
        ...
        for i, df_key in enumerate(['unsep_file_loeps', 'unsep_file_hieps']):            
            ...
            ax.errorbar(df['t'][mask][non_zero_mask], ratios, yerr=errors, marker=markers[i], linestyle='None', label=legend_label, color=colors[i], markeredgecolor=colors[i], markerfacecolor='none', capsize=2)

            # Fit the data using exponential function
            #popt, _ = curve_fit(exp_func, df['Q2'], ratios)
            #fit_line = exp_func(df['t'], *popt)
            #ax.plot(df['t'], fit_line, linestyle='-', color=colors[i], label="{0} Fit: t($\phi$) = {1:.2f}e^({2:.2f}t)".format(df_key, popt[0], popt[1]))
    ...
    for k in range(NumPhiBins):
        ...
        # Create a figure and axis objects for th_cm plot
        fig, axes = plt.subplots(1, 1, figsize=(12, 8), sharex=True)
        ax = axes
        ax.set_title("$\phi$={:.1f}, $Q^2$={:.1f}, W={:.2f}".format(phi_bin_centers[k], float(Q2.replace("p",".")), float(W.replace("p","."))), fontsize=24)
        ...
        for i, df_key in enumerate(['unsep_file_loeps', 'unsep_file_hieps']):            
            ...
            ax.errorbar(df['th_cm'][mask][non_zero_mask], ratios, yerr=errors, marker=markers[i], linestyle='None', label=legend_label, color=colors[i], markeredgecolor=colors[i], markerfacecolor='none', capsize=2)

            # Fit the data using exponential function
            #popt, _ = curve_fit(exp_func, df['th_cm'], ratios)
            #fit_line = exp_func(df['th_cm'], *popt)
