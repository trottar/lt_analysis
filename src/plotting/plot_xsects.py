...
# Description:
# ================================================================
# Time-stamp: "2024-03-23 10:45:16 trottar"
# ================================================================
#
...
with PdfPages(outputpdf) as pdf:
    ...
    for k in range(NumtBins):
        ...
        for i, df_key in enumerate(['unsep_file_loeps', 'unsep_file_hieps']):
            df = file_df_dict[df_key]
            mask =  (df['t'][k*NumPhiBins+int(i/NumPhiBins)] == df['t'])
            ax.scatter(df['th_cm'][mask], df['x_mod'][mask], marker=markers[i], linestyle='None', label=['Low $epsilon$ model', 'High $epsilon$ model'], facecolors='none' , edgecolors=colors[i])
    ...
    for k in range(NumtBins):
        ...
        for i, df_key in enumerate(['unsep_file_loeps', 'unsep_file_hieps']):
            df = file_df_dict[df_key]
            mask =  (df['t'][k*NumPhiBins+int(i/NumPhiBins)] == df['t'])
            ax.scatter(df['phi'][mask], df['x_mod'][mask], marker=markers[i], linestyle='None', label=['Low $epsilon$ model', 'High $epsilon$ model'], facecolors='none' , edgecolors=colors[i])
