...
c         open(77,file='fit_params/it'//prv_it//'/par.'//pol//'_060')
         open(77,file='parameters/par.'//pol//'_'//q2set//
     1        '/par.'//pol//'_'//q2set)
         do while(.true.)
            read(77,*,end=1) p,e,i
            print*,p,e,i
...
c Fit sigL
      ...
      real WW,QQ2
      dimension tm(6),x(6),dx(6),W(6),Q2(6)
      save tm,x,dx,W,Q2
...
c      pause
     ...
     1           sigTT,dsigTT,sigLT,dsigLT,x2,t
            ...
            if (i.ge.tlow) then
               ...
               W(itot)=WW
               Q2(itot)=QQ2
               weight(itot)=weigh(i,pol_set,q2_set)
...
* Revised for IT26, 12.11.09
         ...
         prv_sigTT=(prv_par(12)*qsq*exp(-qsq))*ft*(sin(th_pq(i))**2)*g(W(i))
