...
# Description:
# ================================================================
# Time-stamp: "2023-02-19 15:38:52 trottar"
# ================================================================
#
...
def defineHists(phi_setting, inpDict):    
    ...
    H_hsdelta_SIMC  = ROOT.TH1D("hsdelta","HMS Delta", 200, -20.0, 20.0)
    H_hsxptar_SIMC  = ROOT.TH1D("hsxptar","HMS xptar", 200, -0.1, 0.1)
    H_hsyptar_SIMC  = ROOT.TH1D("hsyptar","HMS yptar", 200, -0.1, 0.1)
    H_ssxfp_SIMC    = ROOT.TH1D("ssxfp","SHMS xfp", 200, -25.0, 25.0)
    H_ssyfp_SIMC    = ROOT.TH1D("ssyfp","SHMS yfp", 200, -25.0, 25.0)
    H_ssxpfp_SIMC   = ROOT.TH1D("ssxpfp","SHMS xpfp", 200, -0.09, 0.09)
    H_ssypfp_SIMC   = ROOT.TH1D("ssypfp","SHMS ypfp", 200, -0.05, 0.04)
    H_hsxfp_SIMC    = ROOT.TH1D("hsxfp","HMS xfp", 200, -40.0, 40.0)
    H_hsyfp_SIMC    = ROOT.TH1D("hsyfp","HMS yfp", 200, -20.0, 20.0)
    H_hsxpfp_SIMC   = ROOT.TH1D("hsxpfp","HMS xpfp", 200, -0.09, 0.05)
    H_hsypfp_SIMC   = ROOT.TH1D("hsypfp","HMS ypfp", 200, -0.05, 0.04)
    H_ssdelta_SIMC  = ROOT.TH1D("ssdelta","SHMS delta", 200, -20.0, 20.0)
    H_ssxptar_SIMC  = ROOT.TH1D("ssxptar","SHMS xptar", 200, -0.1, 0.1)
    H_ssyptar_SIMC  = ROOT.TH1D("ssyptar","SHMS yptar", 200, -0.04, 0.04)
    H_q_SIMC        = ROOT.TH1D("q","q", 200, 0.0, 10.0)
    H_Q2_SIMC       = ROOT.TH1D("Q2","Q2", 200, Q2min, Q2max)
    H_W_SIMC  = ROOT.TH1D("W","W ", 200, Wmin, Wmax)
    H_t_SIMC       = ROOT.TH1D("t","-t", 200, -1.0, 1.5)  
    H_epsilon_SIMC  = ROOT.TH1D("epsilon","epsilon", 200, 0., 1.0)
    H_MM_SIMC  = ROOT.TH1D("MM","MM_{K}", 200, 0.0, 2.0)
    H_th_SIMC  = ROOT.TH1D("th","X' tar", 200, -0.1, 0.1)
    H_ph_SIMC  = ROOT.TH1D("ph","Y' tar", 200, -0.1, 0.1)
    H_ph_q_SIMC  = ROOT.TH1D("ph_q","Phi Detected (ph_xq)", 200, -10.0, 10.0)
    H_th_q_SIMC  = ROOT.TH1D("th_q","Theta Detected (th_xq)", 200, -0.2, 0.2)
    H_ph_recoil_SIMC  = ROOT.TH1D("ph_recoil","Phi Recoil (ph_bq)", 200, -10.0, 10.0)
    H_th_recoil_SIMC  = ROOT.TH1D("th_recoil","Theta Recoil (th_bq)", 200, -10.0, 10.0)
    H_pmiss_SIMC  = ROOT.TH1D("pmiss","pmiss", 200, 0.0, 10.0)
    H_emiss_SIMC  = ROOT.TH1D("emiss","emiss", 200, 0.0, 10.0)
    H_pmx_SIMC  = ROOT.TH1D("pmx","pmx", 200, -10.0, 10.0)
    H_pmy_SIMC  = ROOT.TH1D("pmy","pmy ", 200, -10.0, 10.0)
    H_pmz_SIMC  = ROOT.TH1D("pmz","pmz", 200, -10.0, 10.0)
    H_yield_SIMC = ROOT.TH1D("yield", "Simc Yield", NumtBins*NumPhiBins, 0, 1.0)

    H_hsdelta_DATA  = ROOT.TH1D("hsdelta","HMS Delta", 200, -20.0, 20.0)
    H_hsxptar_DATA  = ROOT.TH1D("hsxptar","HMS xptar", 200, -0.1, 0.1)
    H_hsyptar_DATA  = ROOT.TH1D("hsyptar","HMS yptar", 200, -0.1, 0.1)
    H_ssxfp_DATA    = ROOT.TH1D("ssxfp","SHMS xfp", 200, -25.0, 25.0)
    H_ssyfp_DATA    = ROOT.TH1D("ssyfp","SHMS yfp", 200, -25.0, 25.0)
    H_ssxpfp_DATA   = ROOT.TH1D("ssxpfp","SHMS xpfp", 200, -0.09, 0.09)
    H_ssypfp_DATA   = ROOT.TH1D("ssypfp","SHMS ypfp", 200, -0.05, 0.04)
    H_hsxfp_DATA    = ROOT.TH1D("hsxfp","HMS xfp", 200, -40.0, 40.0)
    H_hsyfp_DATA    = ROOT.TH1D("hsyfp","HMS yfp", 200, -20.0, 20.0)
    H_hsxpfp_DATA   = ROOT.TH1D("hsxpfp","HMS xpfp", 200, -0.09, 0.05)
    H_hsypfp_DATA   = ROOT.TH1D("hsypfp","HMS ypfp", 200, -0.05, 0.04)
    H_ssdelta_DATA  = ROOT.TH1D("ssdelta","SHMS delta", 200, -20.0, 20.0)
    H_ssxptar_DATA  = ROOT.TH1D("ssxptar","SHMS xptar", 200, -0.1, 0.1)
    H_ssyptar_DATA  = ROOT.TH1D("ssyptar","SHMS yptar", 200, -0.04, 0.04)
    H_q_DATA        = ROOT.TH1D("q","q", 200, 0.0, 10.0)
    H_Q2_DATA       = ROOT.TH1D("Q2","Q2", 200, Q2min, Q2max)
    H_W_DATA  = ROOT.TH1D("W","W ", 200, Wmin, Wmax)
    H_t_DATA       = ROOT.TH1D("t","-t", 200, -1.0, 1.5)  
    H_epsilon_DATA  = ROOT.TH1D("epsilon","epsilon", 200, 0., 1.0)
    H_MM_DATA  = ROOT.TH1D("MM","MM_{K}", 200, 0.0, 2.0)
    H_th_DATA  = ROOT.TH1D("th","X' tar", 200, -0.1, 0.1)
    H_ph_DATA  = ROOT.TH1D("ph","Y' tar", 200, -0.1, 0.1)
    H_ph_q_DATA  = ROOT.TH1D("ph_q","Phi Detected (ph_xq)", 200, -10.0, 10.0)
    H_th_q_DATA  = ROOT.TH1D("th_q","Theta Detected (th_xq)", 200, -0.2, 0.2)
    H_ph_recoil_DATA  = ROOT.TH1D("ph_recoil","Phi Recoil (ph_bq)", 200, -10.0, 10.0)
    H_th_recoil_DATA  = ROOT.TH1D("th_recoil","Theta Recoil (th_bq)", 200, -10.0, 10.0)
    H_pmiss_DATA  = ROOT.TH1D("pmiss","pmiss", 200, 0.0, 10.0)
    H_emiss_DATA  = ROOT.TH1D("emiss","emiss", 200, 0.0, 10.0)
    H_pmx_DATA  = ROOT.TH1D("pmx","pmx", 200, -10.0, 10.0)
    H_pmy_DATA  = ROOT.TH1D("pmy","pmy ", 200, -10.0, 10.0)
    H_pmz_DATA  = ROOT.TH1D("pmz","pmz", 200, -10.0, 10.0)
    H_ct_ep_DATA = ROOT.TH1D("ct_ep", "Electron-Proton CTime", 200, -10, 10)
    H_cal_etottracknorm_DATA = ROOT.TH1D("cal_etottracknorm", "HMS Cal etottracknorm", 200, 0.2, 1.8)
    H_cer_npeSum_DATA = ROOT.TH1D("cer_npeSum", "HMS Cer Npe Sum", 200, 0, 30)
    P_cal_etottracknorm_DATA = ROOT.TH1D("P_cal_etottracknorm", "SHMS Cal etottracknorm", 200, 0, 1)
    P_hgcer_npeSum_DATA = ROOT.TH1D("P_hgcer_npeSum", "SHMS HGCer Npe Sum", 200, 0, 50)
    P_aero_npeSum_DATA = ROOT.TH1D("P_aero_npeSum", "SHMS Aero Npe Sum", 200, 0, 50)
    H_phibins_DATA = ROOT.TH1D("phibins", "Phi Bins", NumPhiBins, 0, 1.0)
    H_tbins_DATA = ROOT.TH1D("tbins", "t Bins", NumtBins, 0, 1.0)
    H_yield_DATA = ROOT.TH1D("yield", "Data Yield", NumtBins*NumPhiBins, 0, 1.0)

    H_hsdelta_DUMMY  = ROOT.TH1D("hsdelta","HMS Delta", 200, -20.0, 20.0)
    H_hsxptar_DUMMY  = ROOT.TH1D("hsxptar","HMS xptar", 200, -0.1, 0.1)
    H_hsyptar_DUMMY  = ROOT.TH1D("hsyptar","HMS yptar", 200, -0.1, 0.1)
    H_ssxfp_DUMMY    = ROOT.TH1D("ssxfp","SHMS xfp", 200, -25.0, 25.0)
    H_ssyfp_DUMMY    = ROOT.TH1D("ssyfp","SHMS yfp", 200, -25.0, 25.0)
    H_ssxpfp_DUMMY   = ROOT.TH1D("ssxpfp","SHMS xpfp", 200, -0.09, 0.09)
    H_ssypfp_DUMMY   = ROOT.TH1D("ssypfp","SHMS ypfp", 200, -0.05, 0.04)
    H_hsxfp_DUMMY    = ROOT.TH1D("hsxfp","HMS xfp", 200, -40.0, 40.0)
    H_hsyfp_DUMMY    = ROOT.TH1D("hsyfp","HMS yfp", 200, -20.0, 20.0)
    H_hsxpfp_DUMMY   = ROOT.TH1D("hsxpfp","HMS xpfp", 200, -0.09, 0.05)
    H_hsypfp_DUMMY   = ROOT.TH1D("hsypfp","HMS ypfp", 200, -0.05, 0.04)
    H_ssdelta_DUMMY  = ROOT.TH1D("ssdelta","SHMS delta", 200, -20.0, 20.0)
    H_ssxptar_DUMMY  = ROOT.TH1D("ssxptar","SHMS xptar", 200, -0.1, 0.1)
    H_ssyptar_DUMMY  = ROOT.TH1D("ssyptar","SHMS yptar", 200, -0.04, 0.04)
    H_q_DUMMY        = ROOT.TH1D("q","q", 200, 0.0, 10.0)
    H_Q2_DUMMY       = ROOT.TH1D("Q2","Q2", 200, Q2min, Q2max)
    H_W_DUMMY  = ROOT.TH1D("W","W ", 200, Wmin, Wmax)
    H_t_DUMMY       = ROOT.TH1D("t","-t", 200, -1.0, 1.5)  
    H_epsilon_DUMMY  = ROOT.TH1D("epsilon","epsilon", 200, 0., 1.0)
    H_MM_DUMMY  = ROOT.TH1D("MM","MM_{K}", 200, 0.0, 2.0)
    H_th_DUMMY  = ROOT.TH1D("th","X' tar", 200, -0.1, 0.1)
    H_ph_DUMMY  = ROOT.TH1D("ph","Y' tar", 200, -0.1, 0.1)
    H_ph_q_DUMMY  = ROOT.TH1D("ph_q","Phi Detected (ph_xq)", 200, -10.0, 10.0)
    H_th_q_DUMMY  = ROOT.TH1D("th_q","Theta Detected (th_xq)", 200, -0.2, 0.2)
    H_ph_recoil_DUMMY  = ROOT.TH1D("ph_recoil","Phi Recoil (ph_bq)", 200, -10.0, 10.0)
    H_th_recoil_DUMMY  = ROOT.TH1D("th_recoil","Theta Recoil (th_bq)", 200, -10.0, 10.0)
    H_pmiss_DUMMY  = ROOT.TH1D("pmiss","pmiss", 200, 0.0, 10.0)
    H_emiss_DUMMY  = ROOT.TH1D("emiss","emiss", 200, 0.0, 10.0)
    H_pmx_DUMMY  = ROOT.TH1D("pmx","pmx", 200, -10.0, 10.0)
    H_pmy_DUMMY  = ROOT.TH1D("pmy","pmy ", 200, -10.0, 10.0)
    H_pmz_DUMMY  = ROOT.TH1D("pmz","pmz", 200, -10.0, 10.0)
    H_ct_ep_DUMMY = ROOT.TH1D("ct_ep", "Electron-Proton CTime", 200, -10, 10)
    
    H_hsdelta_RAND  = ROOT.TH1D("hsdelta","HMS Delta", 200, -20.0, 20.0)
    H_hsxptar_RAND  = ROOT.TH1D("hsxptar","HMS xptar", 200, -0.1, 0.1)
    H_hsyptar_RAND  = ROOT.TH1D("hsyptar","HMS yptar", 200, -0.1, 0.1)
    H_ssxfp_RAND    = ROOT.TH1D("ssxfp","SHMS xfp", 200, -25.0, 25.0)
    H_ssyfp_RAND    = ROOT.TH1D("ssyfp","SHMS yfp", 200, -25.0, 25.0)
    H_ssxpfp_RAND   = ROOT.TH1D("ssxpfp","SHMS xpfp", 200, -0.09, 0.09)
    H_ssypfp_RAND   = ROOT.TH1D("ssypfp","SHMS ypfp", 200, -0.05, 0.04)
    H_hsxfp_RAND    = ROOT.TH1D("hsxfp","HMS xfp", 200, -40.0, 40.0)
    H_hsyfp_RAND    = ROOT.TH1D("hsyfp","HMS yfp", 200, -20.0, 20.0)
    H_hsxpfp_RAND   = ROOT.TH1D("hsxpfp","HMS xpfp", 200, -0.09, 0.05)
    H_hsypfp_RAND   = ROOT.TH1D("hsypfp","HMS ypfp", 200, -0.05, 0.04)
    H_ssdelta_RAND  = ROOT.TH1D("ssdelta","SHMS delta", 200, -20.0, 20.0)
    H_ssxptar_RAND  = ROOT.TH1D("ssxptar","SHMS xptar", 200, -0.1, 0.1)
    H_ssyptar_RAND  = ROOT.TH1D("ssyptar","SHMS yptar", 200, -0.04, 0.04)
    H_q_RAND        = ROOT.TH1D("q","q", 200, 0.0, 10.0)
    H_Q2_RAND       = ROOT.TH1D("Q2","Q2", 200, Q2min, Q2max)
    H_W_RAND  = ROOT.TH1D("W","W ", 200, Wmin, Wmax)
    H_t_RAND       = ROOT.TH1D("t","-t", 200, -1.0, 1.5)
    H_epsilon_RAND  = ROOT.TH1D("epsilon","epsilon", 200, 0., 1.0)
    H_MM_RAND  = ROOT.TH1D("MM","MM_{K}", 200, 0.0, 2.0)
    H_th_RAND  = ROOT.TH1D("th","X' tar", 200, -0.1, 0.1)
    H_ph_RAND  = ROOT.TH1D("ph","Y' tar", 200, -0.1, 0.1)
    H_ph_q_RAND  = ROOT.TH1D("ph_q","Phi Detected (ph_xq)", 200, -10.0, 10.0)
    H_th_q_RAND  = ROOT.TH1D("th_q","Theta Detected (th_xq)", 200, -0.2, 0.2)
    H_ph_recoil_RAND  = ROOT.TH1D("ph_recoil","Phi Recoil (ph_bq)", 200, -10.0, 10.0)
    H_th_recoil_RAND  = ROOT.TH1D("th_recoil","Theta Recoil (th_bq)", 200, -10.0, 10.0)
    H_pmiss_RAND  = ROOT.TH1D("pmiss","pmiss", 200, 0.0, 10.0)
    H_emiss_RAND  = ROOT.TH1D("emiss","emiss", 200, 0.0, 10.0)
    H_pmx_RAND  = ROOT.TH1D("pmx","pmx", 200, -10.0, 10.0)
    H_pmy_RAND  = ROOT.TH1D("pmy","pmy ", 200, -10.0, 10.0)
    H_pmz_RAND  = ROOT.TH1D("pmz","pmz", 200, -10.0, 10.0)
    H_ct_ep_RAND = ROOT.TH1D("ct_ep", "Electron-Proton CTime", 200, -10, 10)

    H_hsdelta_DUMMY_RAND  = ROOT.TH1D("hsdelta","HMS Delta", 200, -20.0, 20.0)
    H_hsxptar_DUMMY_RAND  = ROOT.TH1D("hsxptar","HMS xptar", 200, -0.1, 0.1)
    H_hsyptar_DUMMY_RAND  = ROOT.TH1D("hsyptar","HMS yptar", 200, -0.1, 0.1)
    H_ssxfp_DUMMY_RAND    = ROOT.TH1D("ssxfp","SHMS xfp", 200, -25.0, 25.0)
    H_ssyfp_DUMMY_RAND    = ROOT.TH1D("ssyfp","SHMS yfp", 200, -25.0, 25.0)
    H_ssxpfp_DUMMY_RAND   = ROOT.TH1D("ssxpfp","SHMS xpfp", 200, -0.09, 0.09)
    H_ssypfp_DUMMY_RAND   = ROOT.TH1D("ssypfp","SHMS ypfp", 200, -0.05, 0.04)
    H_hsxfp_DUMMY_RAND    = ROOT.TH1D("hsxfp","HMS xfp", 200, -40.0, 40.0)
    H_hsyfp_DUMMY_RAND    = ROOT.TH1D("hsyfp","HMS yfp", 200, -20.0, 20.0)
    H_hsxpfp_DUMMY_RAND   = ROOT.TH1D("hsxpfp","HMS xpfp", 200, -0.09, 0.05)
    H_hsypfp_DUMMY_RAND   = ROOT.TH1D("hsypfp","HMS ypfp", 200, -0.05, 0.04)
    H_ssdelta_DUMMY_RAND  = ROOT.TH1D("ssdelta","SHMS delta", 200, -20.0, 20.0)
    H_ssxptar_DUMMY_RAND  = ROOT.TH1D("ssxptar","SHMS xptar", 200, -0.1, 0.1)
    H_ssyptar_DUMMY_RAND  = ROOT.TH1D("ssyptar","SHMS yptar", 200, -0.04, 0.04)
    H_q_DUMMY_RAND        = ROOT.TH1D("q","q", 200, 0.0, 10.0)
    H_Q2_DUMMY_RAND       = ROOT.TH1D("Q2","Q2", 200, Q2min, Q2max)
    H_W_DUMMY_RAND  = ROOT.TH1D("W","W ", 200, Wmin, Wmax)
    H_t_DUMMY_RAND       = ROOT.TH1D("t","-t", 200, -1.0, 1.5)
    H_epsilon_DUMMY_RAND  = ROOT.TH1D("epsilon","epsilon", 200, 0., 1.0)
    H_MM_DUMMY_RAND  = ROOT.TH1D("MM","MM_{K}", 200, 0.0, 2.0)
    H_th_DUMMY_RAND  = ROOT.TH1D("th","X' tar", 200, -0.1, 0.1)
    H_ph_DUMMY_RAND  = ROOT.TH1D("ph","Y' tar", 200, -0.1, 0.1)
    H_ph_q_DUMMY_RAND  = ROOT.TH1D("ph_q","Phi Detected (ph_xq)", 200, -10.0, 10.0)
    H_th_q_DUMMY_RAND  = ROOT.TH1D("th_q","Theta Detected (th_xq)", 200, -0.2, 0.2)
    H_ph_recoil_DUMMY_RAND  = ROOT.TH1D("ph_recoil","Phi Recoil (ph_bq)", 200, -10.0, 10.0)
    H_th_recoil_DUMMY_RAND  = ROOT.TH1D("th_recoil","Theta Recoil (th_bq)", 200, -10.0, 10.0)
    H_pmiss_DUMMY_RAND  = ROOT.TH1D("pmiss","pmiss", 200, 0.0, 10.0)
    H_emiss_DUMMY_RAND  = ROOT.TH1D("emiss","emiss", 200, 0.0, 10.0)
    H_pmx_DUMMY_RAND  = ROOT.TH1D("pmx","pmx", 200, -10.0, 10.0)
    H_pmy_DUMMY_RAND  = ROOT.TH1D("pmy","pmy ", 200, -10.0, 10.0)
    H_pmz_DUMMY_RAND  = ROOT.TH1D("pmz","pmz", 200, -10.0, 10.0)
    H_ct_ep_DUMMY_RAND = ROOT.TH1D("ct_ep", "Electron-Proton CTime", 200, -10, 10)
