"""Focused D.6 through D.11 and E.2 through E.7.2 procedure-PDF tests."""
...
def _assert_full_background_manifest_contract(test_case, manifest):
    ...
    for page in manifest:
        ...
        if page_id in E72_FULL_BACKGROUND_PAGE_IDS:
            test_case.assertEqual(scope, "setting")
...
class _FakeLegend:
    ...
    def Draw(self):
        self.drawn_on = getattr(getattr(self, "_root", None), "active_pad", None)
        return None
...
class _E2DisplayHistogram:
    ...
    def Draw(self, option):
        if self.root is not None:
            self.drawn_on = getattr(self.root, "active_pad", None)
            self.root.drawn_histograms.append((self, str(option)))
...
def _e4_fixture():
    ...
    e2 = {
        ...
        "coordinate_fingerprint": coordinate_fingerprint,
        "host_state": "proton_cleaned", "source_target_state": "post_proton_noRF",
        "host_label": "Proton-cleaned kaon sample",
...
def _e6_fixture():
    """Return matching detached E.3/E.4 payloads on the standard frozen lattice."""
    diagnostic, method_a = _e3_fixture()
    d11, e2 = _e4_fixture()
    coordinate_fingerprint = d11["coordinate_fingerprint"]
    diagnostic = deepcopy(diagnostic)
    method_a = deepcopy(method_a)
    diagnostic["coordinate_fingerprint"] = coordinate_fingerprint
    diagnostic["records"] = {
        side: tuple({
            **row,
            "coordinate_fingerprint": coordinate_fingerprint,
        } for row in rows)
        for side, rows in diagnostic["records"].items()
    }
    method_a["coordinate_fingerprint"] = coordinate_fingerprint
...
def _e72_fixture():
    """Return exactly linked stored E.7 presentation and E.7.1 map fixtures."""
    e7 = _e7_render_payload()
    parents, cells = [], []
    for group in e7["per_t"]:
        t_index = group["t_index"]
        parents.append({
            "t_index": t_index,
            "t_low": group["t_low"],
            "t_high": group["t_high"],
            "refinable_cell_count": sum(
                cell["prototype_status"] == "available" for cell in group["cells"]
            ),
            "normalization_divisor": 1.125,
            "canonical_baseline_before": -5.0 + t_index,
            "canonical_baseline_after": -5.0 + t_index,
            "outside_delta_baseline": 0.25 * t_index,
            "closure_difference": 0.0,
            "closure_tolerance": 1.0e-12,
            "closure_passed": True,
            "parent_status": "available_parent_preserved",
            "parent_reason": None,
        })
        for source in group["cells"]:
            cells.append({
                ...
                "e7_prototype_status": source["prototype_status"],
                "e7_prototype_reason": source["prototype_reason"],
                "e7_raw_relative_scale": source["prototype_relative_scale"],
                "e7_comparison_availability": source["comparison"]["availability"],
                "refinable": source["prototype_status"] == "available",
                "baseline_record_count": 9,
                "baseline_signed_sum": -0.5,
                "baseline_absolute_support": 2.0,
                "baseline_sumw2": 0.75,
                "baseline_neff": 4.0,
                "parent_normalization_divisor": 1.125,
                "C_final": 1.2933333333333332 if source["prototype_status"] == "available" else 1.0,
                "C_final_status": "available_parent_preserved",
                "C_final_reason": None,
                "uncertainty": None,
            })
    correction = {
        ...
        "source_ab_combination_prototype_fingerprint": e7["prototype_fingerprint"],
        "phase_a_contract_fingerprint": e7["phase_a_contract_fingerprint"],
        "coordinate_fingerprint": e7["coordinate_fingerprint"],
        "host_state": e7["host_state"],
        "source_target_state": e7["source_target_state"],
        "t_edges": deepcopy(e7["t_edges"]),
        "delta_edges": deepcopy(e7["delta_edges"]),
        "parents": parents,
        "cells": cells,
    }
    return e7, correction
...
class FullBackgroundSubtractionD6Tests(unittest.TestCase):
    ...
    def test_e4_uses_the_validated_host_label_for_each_supported_host_state(self):
        d11, e2 = _e4_fixture()
        cleaned = plots.build_full_background_subtraction_e4_payload(d11, e2)
        self.assertTrue(cleaned["available"])
        self.assertEqual(cleaned["host_label"], "Proton-cleaned kaon sample")

        d11, e2 = _e4_fixture()
        d11["host_state"] = "identity_no_proton_cleaning"
        e2["host_state"] = "identity_no_proton_cleaning"
        e2["host_label"] = "Kaon-selected sample"
        identity = plots.build_full_background_subtraction_e4_payload(d11, e2)
        self.assertTrue(identity["available"])
        self.assertEqual(identity["host_label"], "Kaon-selected sample")

        root = _E4ROOT()
        manifest, failures = [], []
        plots._render_e4_t_pages(root, "ignored.pdf", identity, identity["per_t"][0], manifest, failures)
        labels = [label for legend in root.legends for label, _option in legend.entries]
        self.assertIn("Kaon-selected sample", labels)
        self.assertNotIn("Proton-cleaned kaon sample", labels)
    ...
    def test_e4_renderer_uses_five_by_two_shapes_and_stored_ratio_text_only(self):
        ...
        self.assertTrue(all((canvas.width, canvas.height) == (1800, 1200) for canvas in root.canvases))
        self.assertTrue(all(canvas.divisions == [] for canvas in root.canvases))
        self.assertEqual(len(root.pads), 12)
        header_pads = [pad for pad in root.pads if "_header_" in pad.name]
        grid_pads = [pad for pad in root.pads if "_grid_" in pad.name]
        self.assertEqual(len(header_pads), 6)
        self.assertEqual(len(grid_pads), 6)
        self.assertTrue(all(pad.coordinates == (0.0, 0.9, 1.0, 1.0) for pad in header_pads))
        self.assertTrue(all(pad.coordinates == (0.0, 0.0, 1.0, 0.9) for pad in grid_pads))
        self.assertTrue(all(pad.divisions == [] for pad in header_pads))
        self.assertTrue(all(pad.divisions == [(5, 2)] for pad in grid_pads))
        ...
        page_headers = [
            pave for pave in root.pave_texts
            if any("Frozen Method A / legacy Method B" in line for line in pave._text)
        ]
        self.assertEqual(len(page_headers), 6)
        self.assertTrue(all(pave.drawn_on in header_pads for pave in page_headers))
        self.assertTrue(all(
            "Qualitative diagnostic only" in " ".join(pave._text)
            and "legacy Method B" in " ".join(pave._text)
            and "SHMS acceptance context" in " ".join(pave._text)
            and "no new correlation metric" in " ".join(pave._text)
            and "no refinement or correction applied" in " ".join(pave._text)
            for pave in page_headers
        ))
        self.assertEqual(
            sum("|t| = [0.0000, 0.2000] GeV^2" in line for line in page_headers[0]._text),
            1,
        )
        self.assertTrue(all(legend.drawn_on in header_pads for legend in root.legends))
        grid_pave_texts = [pave for pave in root.pave_texts if pave not in page_headers]
        self.assertTrue(all(
            isinstance(pave.drawn_on, _E3GridCellPad) and pave.drawn_on.parent in grid_pads
            for pave in grid_pave_texts
        ))
        self.assertTrue(all(
            isinstance(histogram.drawn_on, _E3GridCellPad)
            and histogram.drawn_on.parent in grid_pads
            for histogram in root.histograms
        ))

    def test_e6_copies_only_stored_e3_e4_evidence_without_aliases(self):
        e3, e4, _d11, _e2 = _e6_fixture()
        e3_cells_before = deepcopy([group["cells"] for group in e3["per_t"]])
        e3_templates_before = [
            (group["kaon_template"], group["pion_template"])
            for group in e3["per_t"]
        ]
        e3_metadata_before = {
            key: deepcopy(e3[key])
            for key in (
        ...
        e4_before = deepcopy(e4)
        payload = plots.build_full_background_subtraction_e6_payload(e3, e4)

        self.assertTrue(payload["available"])
        self.assertEqual(payload["schema_version"], plots.E6_PRESENTATION_SCHEMA_VERSION)
        self.assertTrue(payload["non_authoritative"])
        self.assertFalse(payload["production_objects_mutated"])
        self.assertEqual(payload["checkpoint_type"], "human_scientific_readiness_review")
        self.assertEqual(payload["decision"], "not_encoded_in_software")
        self.assertEqual(payload["coordinate_fingerprint"], e4["coordinate_fingerprint"])
        self.assertEqual(payload["e3_method_a_fingerprint"], e3["method_a_fingerprint"])
        self.assertEqual(payload["d11_ab_comparison_fingerprint"], e4["d11_ab_comparison_fingerprint"])
        self.assertEqual(len(payload["per_t"]), 3)
        self.assertEqual(sum(len(group["cells"]) for group in payload["per_t"]), 30)

        source_e4_cell = e4["per_t"][0]["cells"][0]
        source_e3_context = e3["per_t"][0]["cells"][0]["method_a"]
        copied = payload["per_t"][0]["cells"][0]
        self.assertEqual(copied["method_a"], source_e4_cell["method_a"])
        self.assertEqual(copied["method_b"], source_e4_cell["method_b"])
        self.assertEqual(copied["comparison"], source_e4_cell["comparison"])
        self.assertEqual(copied["availability_label"], source_e4_cell["availability_label"])
        self.assertEqual(
            copied["e3_method_a_hgcer_context"],
            {
                key: source_e3_context[key]
                for key in (
                    "t_index", "delta_index", "support_class", "method_A_status",
                    "method_A_reason", "prompt_positive_count", "prompt_low_count",
                    "prompt_control_count", "partition_closure_passed", "f_low",
                    "f_low_low", "f_low_high",
                )
            },
        )
        self.assertNotIn("kaon_rows", copied)
        self.assertNotIn("pion_rows", copied)
        self.assertEqual([group["cells"] for group in e3["per_t"]], e3_cells_before)
        self.assertEqual(
            [(group["kaon_template"], group["pion_template"]) for group in e3["per_t"]],
            e3_templates_before,
        )
        self.assertEqual(
            {key: e3[key] for key in e3_metadata_before}, e3_metadata_before
        )
        self.assertEqual(e4, e4_before)

        copied["method_a"]["candidate"] = 99.0
        copied["e3_method_a_hgcer_context"]["f_low"] = 0.99
        self.assertEqual(e4["per_t"][0]["cells"][0]["method_a"]["candidate"], 1.4)
        self.assertEqual(e3["per_t"][0]["cells"][0]["method_a"]["f_low"], 0.314)

    def test_e6_rejects_parent_contract_geometry_and_grid_failures_locally(self):
        cases = (
            (lambda e3, _e4: e3.update(available=False), "e6_e3_unavailable"),
            (lambda _e3, e4: e4.update(available=False), "e6_e4_unavailable"),
            (lambda e3, _e4: e3.update(schema_version="wrong"), "e6_e3_contract_invalid"),
            (lambda _e3, e4: e4.update(schema_version="wrong"), "e6_e4_contract_invalid"),
            (lambda e3, _e4: e3.update(non_authoritative=False), "e6_parent_authority_invalid"),
            (lambda _e3, e4: e4.update(production_objects_mutated=True), "e6_parent_authority_invalid"),
            (lambda _e3, e4: e4.update(coordinate_fingerprint="wrong"), "e6_parent_provenance_mismatch"),
            (lambda _e3, e4: e4.update(delta_edges=[-10.0, 0.0, 10.0]), "e6_parent_geometry_mismatch"),
            (lambda e3, _e4: e3.update(per_t=e3["per_t"][:-1]), "e6_e3_grid_invalid"),
            (
                lambda _e3, e4: e4["per_t"][0].update(cells=tuple(
                    [deepcopy(e4["per_t"][0]["cells"][0])] + list(e4["per_t"][0]["cells"])
                )),
                "e6_e4_grid_invalid",
            ),
            (
                lambda e3, _e4: e3["per_t"][0]["cells"][0].update(t_low=99.0),
                "e6_e3_cell_geometry_invalid",
            ),
        )
        for mutation, expected_reason in cases:
            with self.subTest(expected_reason=expected_reason):
    ...
    def test_e6_renderer_uses_header_grid_and_stored_evidence_only(self):
        e3, e4, _d11, _e2 = _e6_fixture()
        payload = plots.build_full_background_subtraction_e6_payload(e3, e4)
        payload_before = deepcopy(payload)
        root = _E4ROOT()
        manifest, failures = [], []
        for group in payload["per_t"]:
            plots._render_e6_t_pages(root, "ignored.pdf", payload, group, manifest, failures)
        ...
        self.assertTrue(all(pad.coordinates == (0.0, 0.0, 1.0, 0.9) for pad in grid_pads))
        self.assertTrue(all(pad.divisions == [] for pad in header_pads))
        self.assertTrue(all(pad.divisions == [(5, 2)] and len(pad.cells) == 10 for pad in grid_pads))
        self.assertEqual(
            [(page["page_id"], page["scope"], page["authoritative"]) for page in manifest],
            [(E6_FULL_BACKGROUND_PAGE_IDS[0], "t{}".format(index), False) for index in (1, 2, 3)],
        )
        self.assertEqual(len(root.histograms), 30)
        self.assertTrue(all(histogram.directory == 0 for histogram in root.histograms))
        visible = [line for text in root.drawn_text for line in text]
        self.assertTrue(any("D.11 Method A: unavailable" in line for line in visible))
        self.assertTrue(any("Legacy Method B: unavailable" in line for line in visible))
        self.assertTrue(any(
            "E.3 HGCer: unsupported; unavailable: support_insufficient" in line
            for line in visible
        ))
        self.assertEqual(sum("B/A (stored): 7.25" in line for line in visible), 6)
        self.assertFalse(any("B/A (stored): None" in line for line in visible))
        self.assertFalse(any(line in {"PASS", "FAIL", "READY", "NOT READY"} for line in visible))
        page_headers = [
            pave for pave in root.pave_texts
            if any("A/B-combination readiness evidence" in line for line in pave._text)
        ]
        self.assertEqual(len(page_headers), 3)
        self.assertTrue(all(pave.drawn_on in header_pads for pave in page_headers))
        self.assertTrue(all(
            "Human scientific checkpoint only" in " ".join(pave._text)
            and "No automatic readiness score" in " ".join(pave._text)
            for pave in page_headers
        ))
    ...
    def test_e6_is_a_final_local_append_after_e3_and_e4(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                ("t1", E6_FULL_BACKGROUND_PAGE_IDS[0]),
                ("t2", E6_FULL_BACKGROUND_PAGE_IDS[0]),
                ("t3", E6_FULL_BACKGROUND_PAGE_IDS[0]),
            ],
        )

        e6["delta_edges"] = [-10.0, 0.0, 10.0]
        with patch.object(plots, "_import_root", return_value=object()), patch.object(
            plots, "_render_d11_t_pages"
        ), patch.object(
            plots, "_render_e2_t_pages"
        ...
        ), patch.object(plots, "_render_e6_t_pages") as render_e6:
            isolated = plots.render_full_background_subtraction_procedure_pages(
                "ignored.pdf", unavailable, unavailable, d11_payload=d11,
                e2_payload=e2, e3_payload=e3, e4_payload=e4, e6_payload=e6,
            )
        self.assertIn("E.6 frozen procedure geometry mismatch", isolated["failures"])
        self.assertEqual(
            [(page["scope"], page["page_id"]) for page in isolated["manifest"][-6:]],
            [
                ("t1", E4_FULL_BACKGROUND_PAGE_IDS[0]),
                ("t1", E4_FULL_BACKGROUND_PAGE_IDS[1]),
                ("t2", E4_FULL_BACKGROUND_PAGE_IDS[0]),
    ...
    def test_e72_payload_copies_only_exactly_linked_stored_values(self):
        e7, correction = _e72_fixture()
        e7_before, correction_before = deepcopy(e7), deepcopy(correction)
        payload = plots.build_full_background_subtraction_e72_payload(e7, correction)
        self.assertTrue(payload["available"])
        self.assertEqual(payload["schema_version"], plots.E72_PRESENTATION_SCHEMA_VERSION)
        self.assertTrue(payload["non_authoritative"])
        self.assertFalse(payload["production_objects_mutated"])
        copied = payload["per_t"][0]["cells"][0]
        self.assertEqual(copied["method_a"], e7["per_t"][0]["cells"][0]["method_a"])
        self.assertEqual(copied["method_b"], e7["per_t"][0]["cells"][0]["method_b"])
        self.assertEqual(copied["prototype_relative_scale"], 1.455)
        self.assertEqual(copied["C_final"], 1.2933333333333332)
        self.assertEqual(payload["per_t"][0]["parent"]["canonical_baseline_before"], -5.0)
        self.assertEqual(e7, e7_before)
        self.assertEqual(correction, correction_before)
        copied["method_a"]["candidate"] = 99.0
        self.assertNotEqual(e7["per_t"][0]["cells"][0]["method_a"]["candidate"], 99.0)

    def test_e72_rejects_parent_provenance_geometry_and_grid_locally(self):
        e7, correction = _e72_fixture()
        cases = (
            (lambda value: value.update(available=False), "e72_parent_contract_invalid"),
            (lambda value: value.update(source_ab_combination_prototype_fingerprint="wrong"), "e72_parent_provenance_mismatch"),
            (lambda value: value.update(delta_edges=[-10.0, 0.0, 10.0]), "e72_parent_geometry_mismatch"),
            (lambda value: value["cells"].pop(), "e72_parent_grid_invalid"),
            (lambda value: value["cells"][0].update(C_final="not-finite"), "e72_correction_cell_contract_invalid"),
        )
        for mutation, reason in cases:
            with self.subTest(reason=reason):
                changed = deepcopy(correction)
                mutation(changed)
                payload = plots.build_full_background_subtraction_e72_payload(e7, changed)
                self.assertFalse(payload["available"])
                self.assertEqual(payload["reason"], reason)

    def test_e72_renderer_has_four_final_setting_pages_and_stored_semantics(self):
        e7, correction = _e72_fixture()
        payload = plots.build_full_background_subtraction_e72_payload(e7, correction)
        root = _E7ROOT()
        manifest, failures = [], []
        plots._render_e72_setting_pages(root, "ignored.pdf", payload, manifest, failures)
        self.assertEqual(failures, [])
        self.assertEqual(len(root.canvases), 4)
        self.assertTrue(all((canvas.width, canvas.height) == (1800, 1200) for canvas in root.canvases))
        self.assertEqual(
            [(entry["page_id"], entry["scope"], entry["authoritative"]) for entry in manifest],
            [(page_id, "setting", False) for page_id in E72_FULL_BACKGROUND_PAGE_IDS],
        )
        self.assertTrue(all(histogram.directory == 0 for histogram in root.histograms))
        self.assertTrue(any(line.y1 == 1.0 and line.y2 == 1.0 for line in root.drawn_lines))
        visible = [line for text in root.drawn_text for line in text]
        self.assertTrue(any("No event application" in line for line in visible))
    ...
    def test_e72_is_a_local_final_append_after_e7(self):
        ...
        e6 = plots.build_full_background_subtraction_e6_payload(e3, e4)
        e7, correction = _e72_fixture()
        e72 = plots.build_full_background_subtraction_e72_payload(e7, correction)

        def record_setting(_root, _pdf, _payload, manifest, _failures):
            for page_id in E72_FULL_BACKGROUND_PAGE_IDS:
        ...
        with patch.object(plots, "_import_root", return_value=object()), patch.object(
            ...
            plots, "_render_e4_t_pages", side_effect=_d12_record_phase_pages(E4_FULL_BACKGROUND_PAGE_IDS, set())
        ), patch.object(
            plots, "_render_e6_t_pages", side_effect=_d12_record_phase_pages(E6_FULL_BACKGROUND_PAGE_IDS, set())
        ), patch.object(
            plots, "_render_e7_t_pages", side_effect=_d12_record_phase_pages(E7_FULL_BACKGROUND_PAGE_IDS, set())
        ), patch.object(plots, "_render_e72_setting_pages", side_effect=record_setting):
            rendered = plots.render_full_background_subtraction_procedure_pages(
                "ignored.pdf", unavailable, unavailable, d11_payload=d11, e2_payload=e2,
                e3_payload=e3, e4_payload=e4, e6_payload=e6, e7_payload=e7,
                e72_payload=e72,
            )
        self.assertEqual(
            [entry["page_id"] for entry in rendered["manifest"][-4:]],
            list(E72_FULL_BACKGROUND_PAGE_IDS),
        )
        malformed = deepcopy(e72)
        malformed["delta_edges"] = [-10.0, 0.0, 10.0]
        with patch.object(plots, "_import_root", return_value=object()), patch.object(
            plots, "_render_d11_t_pages"
        ), patch.object(plots, "_render_e2_t_pages"), patch.object(
            plots, "_render_e3_t_pages"
        ), patch.object(plots, "_render_e4_t_pages"), patch.object(
            plots, "_render_e6_t_pages"
        ), patch.object(plots, "_render_e7_t_pages"), patch.object(
            plots, "_render_e72_setting_pages"
        ) as render_e72:
            isolated = plots.render_full_background_subtraction_procedure_pages(
                "ignored.pdf", unavailable, unavailable, d11_payload=d11, e2_payload=e2,
                e3_payload=e3, e4_payload=e4, e6_payload=e6, e7_payload=e7,
                e72_payload=malformed,
            )
        self.assertTrue(any(message.startswith("E.7.2:") for message in isolated["failures"]))
        render_e72.assert_not_called()

    def test_page_manifest_sidecar_is_deterministic_and_detached(self):
        pages = [{"page_id": "full_background.e72.meeting_overview", "scope": "setting", "authoritative": False}]
        failures = ["E.7.2: test failure"]
        payload = plots.build_full_background_subtraction_page_manifest_artifact(
            setting={"phi_setting": "Left", "kinematic_token": "Q4p4W2p74", "epsilon_filename_token": "lowe", "particle_type": "kaon"},
            pdf_basename="Left_kaon_rand_sub_Q4p4W2p74_lowe_full-background-subtraction.pdf",
            pages=pages, renderer_failures=failures,
        )
        self.assertEqual(payload["schema_version"], plots.FULL_BACKGROUND_SUBTRACTION_PAGE_MANIFEST_SCHEMA_VERSION)
        self.assertEqual(
            plots.full_background_subtraction_page_manifest_filename("Left", "Q4p4W2p74", "lowe"),
            "Left_kaon_rand_sub_Q4p4W2p74_lowe_full-background-subtraction-manifest.json",
        )
        payload["pages"][0]["page_id"] = "changed"
        self.assertEqual(pages[0]["page_id"], "full_background.e72.meeting_overview")
    ...
    def test_e4_is_a_final_local_append_after_e3(self):
        ...
        d11 = _d12_cumulative_payload("D.11", t_edges=(0.0, 1.0, 2.0, 3.0))
        e2 = _d12_cumulative_payload("E.2", t_edges=(0.0, 1.0, 2.0, 3.0))
        with patch.object(plots, "_import_root", return_value=object()), patch.object(
            plots, "_render_d11_t_pages"
        ), patch.object(
            plots, "_render_e2_t_pages"
            ...
            rendered = plots.render_full_background_subtraction_procedure_pages(
                "ignored.pdf", unavailable, unavailable, d11_payload=d11,
                e2_payload=e2, e3_payload=e3, e4_payload=e4,
            )
        self.assertEqual(
            [(page["scope"], page["page_id"]) for page in rendered["manifest"]],
            [
    ...
    def test_e4_respects_finalized_d11_and_e2_parent_availability(self):
        ...
        for parent_name in ("D.11", "E.2"):
            with self.subTest(parent_name=parent_name):
                ...
                e2 = _d12_cumulative_payload("E.2")
                e3 = _d12_cumulative_payload("E.3")
                e4 = _d12_cumulative_payload("E.4")
                if parent_name == "D.11":
                    payloads["D.9"]["delta_edges"] = [-10.0, -5.0, 10.0]
                else:
                    e2["delta_edges"] = [-10.0, -5.0, 10.0]
                with patch.object(plots, "_import_root", return_value=object()), patch.object(
                    plots, "_render_d6_t_pages",
                    side_effect=_d12_record_phase_pages(phase_page_ids[0], set()),
                ), patch.object(plots, "_render_d7_t_pages"), patch.object(
                    plots, "_render_d8_t_pages"
                ), patch.object(plots, "_render_d9_t_pages"), patch.object(
                    plots, "_render_d10_t_pages"
                ), patch.object(plots, "_render_d11_t_pages"), patch.object(
                    plots, "_render_e2_t_pages"
                ), patch.object(
                    plots, "_render_e3_t_pages", side_effect=_d12_record_phase_pages(
                        E3_FULL_BACKGROUND_PAGE_IDS, set()
                    )
                ), patch.object(plots, "_render_e4_t_pages") as render_e4:
                    rendered = plots.render_full_background_subtraction_procedure_pages(
                        "ignored.pdf",
                        payloads["D.6"], payloads["D.7"], payloads["D.8"],
                        payloads["D.9"], payloads["D.10"], payloads["D.11"],
                ...
                self.assertTrue(any(
                    page["page_id"] == "full_background.d6.raw_mm"
                    for page in rendered["manifest"]
                ))
                self.assertEqual(
                    [page["page_id"] for page in rendered["manifest"][-2:]],
                    list(E3_FULL_BACKGROUND_PAGE_IDS) * 2,
                )
                self.assertFalse(any(
                    page["page_id"] in E4_FULL_BACKGROUND_PAGE_IDS
                    for page in rendered["manifest"]
                ))
    ...
    def test_static_presentation_and_runtime_contracts(self):
        ...
        for forbidden in (
            "C_A", "C_B", "use_A", "use_B", "combine_AB",
        ...
        e4_source = source[
            ...
            source.index("def _e6_unavailable")
        ]
        for required in (
            ...
            "grid_pad.Divide(5, 2)",
            "ROOT.TPad",
            "_draw_e4_page_header",
            "host_label",
            "B/A (stored)",
            "Normalized absolute support",
            "Qualitative diagnostic only",
            "frozen legacy Method B",
            "no new correlation metric",
            "no refinement or ",
            "correction applied",
        ):
        ...
        self.assertNotIn("_draw_page_header(", e4_source)
        self.assertNotIn("canvas.cd(1)", e4_source)
        ...
        for required in (
            ...
            "grid_pad.Divide(5, 2)",
            "ROOT.TPad",
            "_draw_e6_page_header",
            "Human scientific checkpoint only",
            "E.3 HGCer",
            "Legacy Method B",
            "B/A (stored)",
            "No automatic readiness score",
        ):
            with self.subTest(e6_required=required):
                self.assertIn(required, e6_source)
        self.assertIn("E6_PRESENTATION_SCHEMA_VERSION", source)
        e6_builder_signature = inspect.signature(
            plots.build_full_background_subtraction_e6_payload
        )
        self.assertEqual(tuple(e6_builder_signature.parameters), ("e3_payload", "e4_payload"))
        self.assertTrue(all(
            parameter.kind not in (inspect.Parameter.VAR_POSITIONAL, inspect.Parameter.VAR_KEYWORD)
            for parameter in e6_builder_signature.parameters.values()
        ))
        self.assertEqual(
            procedure_signature.parameters["e6_payload"].kind,
            inspect.Parameter.KEYWORD_ONLY,
        )
        for forbidden in (
            "find_canonical_bin(", "build_pion_hgcer_method_a(",
            "build_pion_hgcer_method_b(", "build_pion_hgcer_ab_comparison(",
            "build_pion_hgcer_event_contract(", "math.log(", "numpy.corrcoef",
            "pearson", "spearman", "chi2", "p_value", "readiness_score",
            "preferred_method", "selected_method", "use_A", "use_B", "combine_AB",
            "C_A", "C_B", "C_final", "refined_pion_weight",
            "applied_refinement_weight", "acceptance_correction",
        ):
            with self.subTest(e6_forbidden=forbidden):
                self.assertNotIn(forbidden, e6_source)
        self.assertNotIn("canvas.Divide", e6_source)
        final_e6_append = cumulative_renderer_source.rindex("if e6_available:")
        self.assertLess(final_e4_append, final_e6_append)
        self.assertIn(
        ...
        e7_source = source[
            source.index("def _e7_unavailable"):
            source.index("def _e72_unavailable")
        ]
        for required in (
        ...
        for required in (
            ...
            "Detached scientific map only",
            "canonical_baseline_before",
            "canonical_baseline_after",
        ):
            with self.subTest(e72_required=required):
                self.assertIn(required, e72_source)
        self.assertIn("full_background_subtraction_page_manifest/v1", source)
        self.assertEqual(
            tuple(inspect.signature(plots.build_full_background_subtraction_e72_payload).parameters),
            ("e7_payload", "parent_preserving_correction"),
        )
        self.assertEqual(
            procedure_signature.parameters["e72_payload"].kind,
            inspect.Parameter.KEYWORD_ONLY,
        )
        for forbidden in (
            "build_pion_hgcer_method_a(", "build_pion_hgcer_method_b(",
            "build_pion_hgcer_ab_combination_prototype(", "math.log(",
            "math.sqrt(", "interpolate", "correlation(", "readiness_score",
            "preferred_method", "selected_method", "use_A", "use_B", "combine_AB",
        ):
            with self.subTest(e72_forbidden=forbidden):
                self.assertNotIn(forbidden, e72_source)
        final_e72_append = cumulative_renderer_source.rindex("if e72_available:")
        self.assertLess(final_e7_append, final_e72_append)
        self.assertIn("_render_e72_setting_pages", cumulative_renderer_source[final_e72_append:])
        ...
        self.assertLess(
            runtime.index("build_full_background_subtraction_e7_payload(", start),
            runtime.index("build_full_background_subtraction_e72_payload(", start),
        )
        self.assertLess(
            runtime.index("build_full_background_subtraction_e72_payload(", start),
        ...
        e6_call_start = runtime.index(
            "build_full_background_subtraction_e6_payload(", start
        )
        e6_call_end = runtime.index(")", e6_call_start) + 1
        e6_call = runtime[e6_call_start:e6_call_end]
        self.assertIn("full_background_subtraction_e3_payload", e6_call)
        self.assertIn("full_background_subtraction_e4_payload", e6_call)
        for forbidden_input in (
            "pion_hgcer_method_a", "pion_hgcer_method_b",
            "pion_hgcer_method_b_comparison", "pion_hgcer_ab_comparison",
            "phase_d_checkpoint", "pion_hgcer_tdelta_diagnostic",
        ):
        ...
        e72_call_start = runtime.index(
            "build_full_background_subtraction_e72_payload(", start
        )
        e72_call_end = runtime.index(")", e72_call_start) + 1
        e72_call = runtime[e72_call_start:e72_call_end]
        self.assertIn("full_background_subtraction_e7_payload", e72_call)
        self.assertIn("pion_hgcer_parent_preserving_correction", e72_call)
        self.assertIn("e72_payload=full_background_subtraction_e72_payload", block)
        self.assertNotIn('histDict["full_background_subtraction_e72', block)
        self.assertLess(
        ...
        self.assertLess(
            ...
            runtime.index("build_full_background_subtraction_page_manifest_artifact(", start),
        )
        self.assertLess(
            runtime.index("build_full_background_subtraction_page_manifest_artifact(", start),
            runtime.index("write_full_background_subtraction_page_manifest_json(", start),
        )
        self.assertLess(
            runtime.index("render_full_background_subtraction_procedure_pages(", start),
            runtime.index("close_full_background_subtraction_pdf(", start),
