...
while getopts 'hsrancjdw:m:A:P:' flag; do
    case "${flag}" in
        ...
        echo "    Replay ROOT output is copied to MSS by SWIF using add-job -output."
        ...
        echo "    applyCuts uploads are still run separately from an interactive"
        echo "    ifarm session, but can be launched from this same wrapper with -j."
        echo "    In Jasmine mode, replay upload is still available manually and"
        echo "    -c switches the upload product kind to skim."
        echo
        echo "Examples..."
